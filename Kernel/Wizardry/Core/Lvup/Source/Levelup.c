...
STATIC_DECLAR void UnitLvup_Fixed(struct BattleUnit *bu, int bonus)
{
	...
	bu->changeHP =
		GetStatIncreaseFixed(GetUnitHpGrowth(unit) + bonus, ref += 5);
	bu->changePow =
		GetStatIncreaseFixed(GetUnitPowGrowth(unit) + bonus, ref += 5);
	bu->changeSkl =
		GetStatIncreaseFixed(GetUnitSklGrowth(unit) + bonus, ref += 5);
	bu->changeSpd =
		GetStatIncreaseFixed(GetUnitSpdGrowth(unit) + bonus, ref += 5);
	bu->changeLck =
		GetStatIncreaseFixed(GetUnitLckGrowth(unit) + bonus, ref += 5);
	bu->changeDef =
		GetStatIncreaseFixed(GetUnitDefGrowth(unit) + bonus, ref += 5);
	bu->changeRes =
		GetStatIncreaseFixed(GetUnitResGrowth(unit) + bonus, ref += 5);

	BU_CHG_MAG(bu) =
		GetStatIncreaseFixed(GetUnitMagGrowth(unit) + bonus, ref += 5);
}
...
{
	static void (*const funcs[])(struct BattleUnit *bu,
				     int bonus) = { [0] = UnitLvup_Vanilla,
						    [1] = UnitLvup_RandC,
						    [2] = UnitLvup_Fixed,
						    [3] = UnitLvup_100,
						    [4] = UnitLvup_0 };
	...
	for (i = 0; i < retry; i++) {
		total_lvup = bu->changeHP + bu->changePow + bu->changeSkl +
			     bu->changeSpd + bu->changeLck + bu->changeDef +
...
void CheckBattleUnitLevelUp(struct BattleUnit *bu)
{
	if (CanBattleUnitGainLevels(bu) && bu->unit.exp >= 100) {
		int bonus = (bu->unit.state & US_GROWTH_BOOST) ?
				    get_metis_tome_growth_bonus() :
				    0;
		...
		TryAddSkillLvup(GetUnitFromCharIdAndFaction(
					UNIT_CHAR_ID(&bu->unit), FACTION_BLUE),
				bu->unit.level);
