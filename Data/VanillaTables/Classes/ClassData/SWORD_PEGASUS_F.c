#include "common-chax.h"
#include "KSDefinitions.h"

const struct ClassData ClassSwordPegasusF = {
    .nameTextId = MSG_CLASS_LANCE_PEGASUS_NAME,
    .descTextId = MSG_CLASS_LANCE_PEGASUS_DESC,
    .number = CLASS_SWORD_PEGASUS_F,
    .promotion = CLASS_FALCON_KNIGHT,
    .SMSId = CLASS_SWORD_PEGASUS_F - 1,
    .defaultPortraitId = 0x86,
    .sort_order = 0x10,

    .baseHP  = 27,
    .basePow = 5,
    .baseSkl = 8,
    .baseSpd = 9,
    .baseDef = 3,
    .baseRes = 9,
    .baseCon = 6,
    .baseMov = 7,

    .maxHP  = 45,
    .maxPow = 17,
    .maxSkl = 20,
    .maxSpd = 19,
    .maxDef = 15,
    .maxRes = 20,
    .maxCon = 25,
    .baseLuck = 9,


    .growthHP  = 80,
    .growthPow = 35,
    .growthSkl = 50,
    .growthSpd = 40,
    .growthDef = 35,
    .growthRes = 65,
    .growthLck = 50,

    .promotionHp  = 0,
    .promotionPow = 0,
    .promotionSkl = 0,
    .promotionSpd = 0,
    .promotionDef = 0,
    .promotionRes = 0,

    .attributes = CA_MOUNTEDAID | CA_CANTO | CA_PEGASUS | CA_FEMALE,

    .baseRanks = {
        [ITYPE_SWORD] = WPN_EXP_E,
    },
    .pBattleAnimDef = PEGASUS_F_REF,
    .pMovCostTable = {
        TerrainTable_MovCost_FlyNormal,
        TerrainTable_MovCost_FlyRain,
        TerrainTable_MovCost_FlySnow,
    },
    .pTerrainAvoidLookup = TerrainTable_Avo_Fly,
    .pTerrainDefenseLookup = TerrainTable_Def_Fly,
    .pTerrainResistanceLookup = TerrainTable_Res_Fly,
};